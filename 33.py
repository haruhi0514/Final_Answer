...
for page in range(1, 20):
    ...
    # 店舗リンク取得（data-idを持つものだけ）
    shops = soup.select("a[href^='https://r.gnavi.co.jp/'][data-id]")
...
for link in shop_links:
    ...
    # 店名
    name_tag = soup.select_one("h2 strong, h1")
...
# CSV出力（Excel用にShift-JIS）
df.to_csv("1-1_sjis.csv", index=False, encoding="cp932")
print("✅ 1-1_sjis.csv を作成しました。提出できます。")
